...
ObjRevolucion::ObjRevolucion(const std::string & archivo, int num_instancias, bool tapa_sup, bool tapa_inf, char eje_r)
{
...
ObjRevolucion::ObjRevolucion(std::vector<Tupla3f> archivo, int num_instancias, bool tapa_sup, bool tapa_inf, char eje_r)
{	
	// Inicializar tablas de vértices y triángulos
...
void ObjRevolucion::crearMalla(std::vector<Tupla3f> perfil, int num_instancias, const bool tapas, char eje_r)
{
	...
	// Extraer polo norte  
	//fabs(perfil.back(0))<epsilon() && fabs(perfil.back(2))<epsilon()
	if(perfil.back()(0)==0 && perfil.back()(2)==0){
		pnorte = perfil.back();
	...
	// Extraer polo sur
	if(perfil.front()(0)==0 && perfil.front()(2)==0){
		psur   = perfil.front();
